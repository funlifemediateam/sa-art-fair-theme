(function () {
  ...
  function init() {
    ...
    /* ── OAuth helpers ──
       Login runs through a popup pointed at our own backend (API_URL), which
       does the Google/Facebook OAuth exchange server-side and posts the
       result back here. Google/Facebook only ever see API_URL as the
       redirect target, never this storefront's domain — so this works on
       any Shopify store the quiz section is installed on, with nothing to
       register per store. See ART PERSONALITY QUIZ / oauth relay in api.js. */
    var GOOGLE_ENABLED = !!window.__saGoogleClientId;
    var FB_ENABLED     = !!window.__saFbAppId;

    function openOAuthPopup(provider) {
      return new Promise(function(resolve, reject) {
        var startUrl = API_URL + '/api/oauth/' + provider + '/start?return=' + encodeURIComponent(window.location.origin);
        var popup = window.open(startUrl, 'sa-quiz-oauth', 'width=480,height=620');
        if (!popup) { reject(new Error('popup-blocked')); return; }

        var apiOrigin;
        try { apiOrigin = new URL(API_URL).origin; } catch (e) { apiOrigin = API_URL; }

        var settled = false;
        var poll = setInterval(function () {
          if (popup.closed) {
            cleanup();
            if (!settled) reject(new Error('popup-closed'));
          }
        }, 500);
        ...
        function onMessage(e) {
          ...
          var data = e.data || {};
          if (!data || data.source !== 'sa-quiz-oauth' || data.provider !== provider) return;
          settled = true;
          cleanup();
          if (data.ok) resolve(data); else reject(new Error(data.error || 'oauth-failed'));
        }
    ...
    function bindOverlayEvents() {
      ...
      if (googleBtn && GOOGLE_ENABLED) {
        googleBtn.onclick = function() {
          googleBtn.disabled = true;
          googleBtn.textContent = 'Connecting…';
          openOAuthPopup('google').then(function(data) {
            handleOAuthSuccess(data.email, data.name, 'google');
          }).catch(function(err) {
            ...
            if (err && err.message === 'popup-blocked') alert('Please allow pop-ups to continue with Google.');
          });
        };
      }
      ...
      if (fbBtn && FB_ENABLED) {
        fbBtn.onclick = function() {
          ...
          openOAuthPopup('facebook').then(function(data) {
            handleOAuthSuccess(data.email, data.name, 'facebook');
          }).catch(function(err) {
            fbBtn.disabled = false; fbBtn.textContent = 'Continue with Facebook';
            if (err && err.message === 'popup-blocked') alert('Please allow pop-ups to continue with Facebook.');
            else if (err && err.message === 'no_email') alert('Facebook did not return your email. Please enter it manually.');
          });
        };
      }
